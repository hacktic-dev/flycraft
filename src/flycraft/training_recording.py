...
class EpisodeRecording:
    def __init__(self,folder,fly,config,game_config,initial,history,preview=False):
        self.folder=Path(folder);self.folder.mkdir(parents=True)
        self.view=Visuals(fly,self.folder,record=config['record_dashboard'],preview=preview)
        ...
        meta=json.loads((self.folder/'visuals.json').read_text());meta.update(learning=fly.learning,model=fly.config['model'])
        (self.folder/'visuals.json').write_text(json.dumps(meta,indent=2))

    def write(self,fly,rgb,control,action,before,after,tick,neural,panel):
        ...
        if self.video:
            frame=np.asarray(fit(Image.fromarray(rgb)))
            for _ in range(3):self.video.send(frame)
        if self.render:self.view.update(rgb,control,tick)
