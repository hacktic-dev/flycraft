"""Gamma1 LTD adapter with the pinned DOOMFLY v6 R1-R6 + R8 RGB visual input."""
...
class LearningFly:
    def __init__(self,config):
        prepare_kernel()
        self.brain=R8VisualMemoryBrain(eta=config['eta'])
        ...
        # Verify the untouched pinned graph first; the v6 visual adapter then
        # applies its declared R8->aMe12 sign assumption to existing edges.
    ...
    def step(self,rgb,record=False):
        ...
        try:
            counts,wall=self.brain.rgb_step(rgb,50.,learning=self.learning,
